...
for i in range(len(mot) - 1):
    for j in range(i + 1, len(mot)):
        ...
        six_mask_turn_on[comb_index][x, y] = 1
        six_mask_turn_on[comb_index][y, x] = 1
...
for data in [dataset_test,dataset_train]:
    ...
    count2 = 0
    count10 = 0
    pbar1 = trange(len(data)/24)
    for ques in pbar1:
        question = data[24 * ques][0]
        key = data[24 * ques][1]
        answer,_,_,_ = get_model_forward(check(data[24 * ques][0]).to(DEVICE))
        if key == answer:
            count += 1
        if key[0] == answer[0]:
            count1 += 1
